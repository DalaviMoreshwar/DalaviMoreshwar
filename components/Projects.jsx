    ...
    <div>
      ...
      <div id="projects" className="w-full my-10">
        ...
        {projectsDetails.map(project=>(
            <div key={project.id} className="shadow-xl bg-slate-50 rounded-xl my-10 dark:bg-gray-800 flex-1">
            <Image className='rounded-xl shadow-xl' loading='lazy' src={project.bgImg} alt={project.title}  />
              <div className='p-3'>

              <h3 className="text-lg text-center font-medium pb-3 text-purple-500">{project.title}</h3>
              ...
              <div className="flex space-x-3 mt-3">
                <span className="bg-gradient-to-tr cursor-pointer from-purple-800 to-rose-500 text-white px-3 py-1 border-none rounded-md">
                  <Link href={project.githubUrl} target="_blank">
                    <div className="flex gap-2"><BsGithub className="m-auto" /> Github</div>
                  </Link>
                </span>
                <span className="bg-gradient-to-tr cursor-pointer from-purple-800 to-green-400 text-white px-3 py-1 border-none rounded-md">
                  <Link href={project.webUrl} target="_blank" >
                    <div className="flex gap-2"><BsLaptop className="m-auto" /> View</div>
                  </Link>
                </span>
              </div>
              ...
              {project.tags.map((tag, index)=>(
                <>
                  <code key={index} className="row-span-3 col-span-2">
                    <span className="bg-[#ccc] p-1 text-sm  rounded  text-gray-800">{tag}</span>
                  </code>  {" "}
                  </>
              ))}       
